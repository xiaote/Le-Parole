...
final class HomeViewModel {
    var stats = HomeStats(mastered: 0, inProgress: 0, reviewsDue: 0, wordsLearnedToday: 0, newAvailable: 0, mistakesToday: [], testQueueCount: 0, recognitionBacklog: 0)
    ...
    var newToLearnToday: Int { min(newAvailable, max(0, dailyGoal - wordsLearnedToday)) }
    ...
    var extraSessionDailyLimit: Int {
        // If the backlog of unrecognized words is high (>= 1.5x daily goal), 
        // don't introduce new words in the extra session. Just drill the backlog.
        if recognitionBacklog >= Int(Double(dailyGoal) * 1.5) {
